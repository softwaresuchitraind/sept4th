class Shared
{
	void test1(Shared obj)
	{
		...
		synchronized(this)
		{
			...
			synchronized(obj)
			{
				System.out.println("test1 syn by " +t1.getName());
			}
	...
	void test2(Shared obj)
	{
		...
		synchronized(obj)
		{
			System.out.println("test2 syn2 by"+ t1.getName());
		}
...
test1 begin by Thread-0
test1 syn by Thread-0
test1 syn1 end by Thread-0
test2 syn2 byThread-1
test1 end byThread-0
test2 end byThread-1
state of a1:TERMINATED
state of b1:TERMINATED

deadlock
--------------
whenever thread is unde deadlock state is under blocked.


-new ,Runnable and Terminated are defalut state.
-wait() and join() results waiting state.
*/
